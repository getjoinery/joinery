#!/bin/bash
# _reconcile_upgradable_assets.sh
#
...
# extensions) but does NOT stream the source's theme/ or plugins/ DIRECTORIES.
# The build context only contains whatever was on the admin machine at
# `install.sh site` time. If the source had upgradable items the admin
...
# before Apache starts, and downloads ONLY items registered in the local DB
# that are marked receives_upgrades=true (plg_receives_upgrades /
# thm_receives_upgrades). Preserved-on-deploy items aren't on the upgrade
# server and must be brought over by whoever did the clone.
#
# On a FRESH install (no clone), the fresh SQL dump has no extension rows
# beyond the joinery-system theme row, so this script does nothing — by
...
if [ -z "$UPGRADE_SOURCE" ]; then
    echo "[sync] upgrade_source not set in database — skipping upgradable asset sync"
    exit 0
fi

echo "[sync] Syncing upgradable assets from ${UPGRADE_SOURCE}..."

# Download upgradable items registered in the local DB that are missing from disk.
...
sync_items() {
    ...
    if [ -z "$names" ]; then
        echo "[sync] No upgradable ${type_label}s registered in site database — nothing to fetch"
        return
    fi
...
sync_items "plugin" "${PUBLIC_HTML}/plugins" \
    "SELECT plg_name FROM plg_plugins WHERE plg_receives_upgrades = true" \
    "&type=plugin"

sync_items "theme"  "${PUBLIC_HTML}/theme" \
    "SELECT thm_name FROM thm_themes WHERE thm_receives_upgrades = true" \
    ""

echo "[sync] Upgradable asset sync complete"
exit 0
