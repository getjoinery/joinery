...
if (!class_exists('PathHelper')) {
	require_once(dirname(__DIR__, 3) . '/includes/PathHelper.php');
}
...
$sql = "SELECT stc_usr_user_id FROM stc_stripe_customers WHERE stc_customer_id_test IS NOT NULL";
...
$sql = "UPDATE stc_stripe_customers SET stc_customer_id_test = NULL WHERE stc_customer_id_test IS NOT NULL";
$q = $dblink->prepare($sql);
$q->execute();
