<?php
	...
	if(!$_GET['print-format']){
		...
		'breadcrumbs' => array(
			'Orders'=>'/plugins/store/admin/admin_orders', 
			'Stripe Orders' => '',
		),
			'session' => $session,
		)
	...
	$formwriter = $page->getFormWriter('form1', [
		'method' => 'GET',
		'action' => '/plugins/store/admin/admin_stripe_orders'
	]);
	...
	echo '<br /><br />';
	if($offset){
		echo '<a href="/plugins/store/admin/admin_stripe_orders?startdate='.$display_startdate.'&enddate='.$display_enddate.'"><< Back to page 1</a>';
	...
	if(count($charges['data']) == $numperpage){
		...
		echo 'Multiple pages of results:  <a href="/plugins/store/admin/admin_stripe_orders?offset='.$last_charge->id.'&startdate='.$display_startdate.'&enddate='.$display_enddate.'&currpage='.$nextpage .'">Next page >></a> ';
	}

	$headers = array('Date', 'Order #', 'Total Amount', 'Billing User', 'Billing Email', 'Address');
	//$altlinks = array('Print format' => '/plugins/store/admin/admin_stripe_orders?print-format=true&startdate='.$display_startdate.'&enddate='.$display_enddate);
	$altlinks = array();
	$box_vars =	array(
		'altlinks' => $altlinks,
	...
	foreach($charges as $charge) {
		...
		if($charge->paid){
			...
			if($charge->metadata['ord_order_id']){
				array_push($rowvalues, '<a href="/plugins/store/admin/admin_order?ord_order_id='.$charge->metadata['ord_order_id'].'">Order '.$charge->metadata['ord_order_id'].'</a> - ' . $description);
			}
			else if($charge->payment_intent){
				$order = Order::GetByStripePaymentIntent($charge->payment_intent); 
				if($order){
					array_push($rowvalues, '<a href="/plugins/store/admin/admin_order?ord_order_id='.$order->key.'">Order '.$order->key.'</a> - ' . $description);
				}
				else{
	...
	//echo '<a style="text-align:center" href="/plugins/store/admin/admin_stripe_orders?offset='.$offset .'">Next 100 >></a>';
