...
function admin_order_edit_logic(array $input): LogicResult {
	...
	if (isset($input['ord_order_id']) || isset($input['ord_order_id'])) {
		$ord_order_id = isset($input['ord_order_id']) ? $input['ord_order_id'] : $input['ord_order_id'];
	...
	if(LibraryFunctions::isFormSubmission()){
		...
		if(!$order->key || !$order->is_stripe_order()){
			$order->set('ord_total_cost', $input['ord_total_cost']);
			$order->set('ord_status', Order::STATUS_PAID);

			// Process datetime from FormWriter V2
			$timestamp = FormWriterV2Base::process_datetimeinput($input, 'ord_timestamp', true);
			if($timestamp !== NULL){
		...
		return LogicResult::redirect('/plugins/store/admin/admin_order?ord_order_id='.$order->key);
	}

	// Load data for display
	$breadcrumbs = array('Orders'=>'/plugins/store/admin/admin_orders');
