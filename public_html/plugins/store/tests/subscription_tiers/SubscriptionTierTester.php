...
class SubscriptionTierTester {
    ...
    private function testFeatureAccess() {
        ...
        // Get feature
        $max_devices = SubscriptionTier::getUserFeature($this->test_user_id, 'dns_filtering_scrolldaddy_max_devices', 1);
    ...
    private function createStripeSubscription($product_id) {
        ...
        try {
            ...
            if (!$stripe_helper->read_customer_id($user)) {
                ...
                $stripe_helper->write_customer_id($user, $customer->id);

                echo "<p>✓ Created Stripe customer: {$customer->id}</p>";
            } else {
                echo "<p>✓ Using existing Stripe customer: {$stripe_helper->read_customer_id($user)}</p>";
            }
            ...
            $stripe_helper->get_stripe_client()->paymentMethods->attach(
                $payment_method->id,
                ['customer' => $stripe_helper->read_customer_id($user)]
            );

            // Set as default payment method
            $stripe_helper->get_stripe_client()->customers->update(
                $stripe_helper->read_customer_id($user),
            ...
            return [
                ...
                'order_id' => $order->key,
                'order_item_id' => $order_item->key,
                'customer_id' => $stripe_helper->read_customer_id($user)
            ];
    ...
    private function cleanupStripeData() {
        ...
        try {
            $user = new User($this->test_user_id, TRUE);
            $stripe_helper = new StripeHelper();
            $stripe_customer_id = $stripe_helper->read_customer_id($user);
            ...
            if ($stripe_customer_id) {
                ...
                $stripe_helper->write_customer_id($user, NULL);
            }
