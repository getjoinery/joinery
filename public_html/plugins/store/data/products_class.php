...
class Product extends SystemBase {
	...
	public static $field_specifications = array(
	    ...
	    'pro_fulfillment_provider' => array('type'=>'varchar(32)', 'is_nullable'=>true),
	    'pro_fulfillment_ref' => array('type'=>'int4', 'is_nullable'=>true),
...
public function get_requirement_info($output='text') {
	...
	function get_product_requirements() {
		...
		// A fulfillment provider may contribute extra requirements to collect at
		// purchase time (e.g. an event's required pre-purchase survey).
		if ($this->get('pro_fulfillment_provider')) {
			require_once(PathHelper::getIncludePath('plugins/store/includes/FulfillmentRegistry.php'));
			$fulfillment_provider = FulfillmentRegistry::get($this->get('pro_fulfillment_provider'));
			if ($fulfillment_provider) {
				foreach ($fulfillment_provider->extraRequirements($this, (int)$this->get('pro_fulfillment_ref')) as $extra) {
					$requirements[] = $extra;
				}
			}
		}
...
class MultiProduct extends SystemMultiBase {
	...
	protected function getMultiResults($only_count = false, $debug = false) {
		...
		if (isset($this->options['fulfillment_provider'])) {
			$filters['pro_fulfillment_provider'] = [$this->options['fulfillment_provider'], PDO::PARAM_STR];
		}

		if (isset($this->options['fulfillment_ref'])) {
			$filters['pro_fulfillment_ref'] = [$this->options['fulfillment_ref'], PDO::PARAM_INT];
		}
