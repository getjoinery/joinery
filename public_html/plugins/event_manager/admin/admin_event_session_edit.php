<?php
	...
	if($_REQUEST['action'] == 'delete'){
		...
		LibraryFunctions::redirect('/plugins/event_manager/admin/admin_event?evt_event_id='.$event->key);
		return;
	}
		...
		if(!$latest_session){
			...
			header("Location: /plugins/event_manager/admin/admin_event?evt_event_id=".$event->key);
			exit();
		}
	...
	$event_session->save();
		...
		LibraryFunctions::redirect('/plugins/event_manager/admin/admin_event?evt_event_id='.$event_session->get('evs_evt_event_id'));
		return;
	}
	...
	array(
		...
		'breadcrumbs' => array('Events'=>'/plugins/event_manager/admin/admin_events', $event->get('evt_name')=>'/plugins/event_manager/admin/admin_event?evt_event_id='.$event->key,'Add Session'=> ''),
		'uploader' => TRUE,
		'session' => $session,
	)
	);
	...
	$formwriter->dropinput('evs_tier_min_level', 'Minimum Tier Required', [
		'options' => $tier_options,
		'helptext' => 'Restrict this session to users with this subscription tier or higher'
	]);
	...
	if($event_session->key){
		...
		$formwriter = $page->getFormWriter('form2', ['action' => '/plugins/event_manager/admin/admin_event_session_edit', 'method' => 'POST']);
