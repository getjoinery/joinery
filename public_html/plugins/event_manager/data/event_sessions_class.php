...
require_once(PathHelper::getIncludePath('plugins/event_manager/data/event_session_files_class.php'));
...
public static function GetBySessionNumber($event_id, $session_number){
	...
	function record_analytic($user_id, $type=1){
		...
		// Analytics rows carry the generic entity pair: entity_type 'event_session'
		// and the session id. The event is recoverable by joining evs_event_sessions.
		$q = $dblink->prepare('INSERT INTO sev_session_analytics (sev_entity_type, sev_entity_id, sev_type, sev_usr_user_id, sev_time) VALUES (?, ?, ?, ?, ?)');
		$q->bindValue(1, 'event_session', PDO::PARAM_STR);
		$q->bindValue(2, $this->key, PDO::PARAM_INT);
	...
	function get_last_visited_session_id_for_user($user_id){
		...
		// Recover "last visited session for this event" by joining the generic
		// analytics rows (entity_type='event_session') back to their sessions.
		$q = $dblink->prepare('SELECT sev.sev_entity_id FROM sev_session_analytics sev
			JOIN evs_event_sessions evs ON evs.evs_event_session_id = sev.sev_entity_id
			WHERE sev.sev_usr_user_id=? AND sev.sev_entity_type=\'event_session\' AND evs.evs_evt_event_id=?
			ORDER BY sev.sev_entity_id DESC');
		...
		return $results ? $results->sev_entity_id : null;
	}
	...
	function get_last_visited_time_for_user($user_id){
		...
		$q = $dblink->prepare('SELECT sev_time FROM sev_session_analytics WHERE sev_usr_user_id=? AND sev_entity_type=\'event_session\' AND sev_entity_id=? ORDER BY sev_time DESC');
		$q->bindValue(1, $user_id, PDO::PARAM_INT);
		$q->bindValue(2, $this->key, PDO::PARAM_INT);
	...
	function get_number_visits_for_user($user_id){
		...
		$q = $dblink->prepare('SELECT count(*) as totalcount FROM sev_session_analytics WHERE sev_usr_user_id=? AND sev_entity_type=\'event_session\' AND sev_entity_id=?');
		$q->bindValue(1, $user_id, PDO::PARAM_INT);
		$q->bindValue(2, $this->key, PDO::PARAM_INT);
