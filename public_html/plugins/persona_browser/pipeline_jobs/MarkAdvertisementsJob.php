 ...
 * PersonaBlockedSender row that hides them from the feed page. Allowed senders
 * (PersonaAllowedSender) are never judged and never blocked.
 *
...
class MarkAdvertisementsJob implements PipelineJobInterface {
    ...
    /**
     * A blocked sender's posts can never be shown, and an allowed sender's
     * posts are shown whatever the verdict — so judging either would spend AI
     * verdicts on nothing, and both work queries skip them. Lower-cased match,
     * same as the feed page's display filter.
     */
    private static function notRuledClause(): string {
        return "NOT EXISTS (SELECT 1 FROM pbs_persona_blocked_senders
                ...
                AND NOT EXISTS (SELECT 1 FROM pas_persona_allowed_senders
                            WHERE pas_owner_user_id = pfi_owner_user_id
                              AND pas_persona = pfi_persona
                              AND pas_delete_time IS NULL
                              AND lower(pas_author) = lower(pfi_author))";
    }
...
commentary, community/group discussion, and someone simply mentioning a
product in a non-promotional way.

EVENTS ARE NEVER ADVERTISEMENTS. A post announcing, inviting to, or reminding
about an event — a concert, show, class, workshop, meetup, festival, market,
dance, party, fundraiser, talk, game, or gathering of any kind — is is_ad =
false, even when it names a venue, a date, a ticket price, a cover charge, an
RSVP or ticket link, or comes from the organizer, the venue, or a promoter.
Selling a ticket to an event is inviting people to the event, not advertising
a product. Give the reason as "event" plus what kind, e.g. "event: workshop".
...
PROMPT;
     ...
     * author is on the allow list, so automation never overrides that call.
     */
