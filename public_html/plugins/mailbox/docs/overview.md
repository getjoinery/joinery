...
A one-line **summary** per message, generated by the `email_triage` pipeline
job (see `plugins/joinery_ai/docs/overview.md` § Registered jobs) — the
recipe is called "Email summaries" on the recipes page and in the reader's
AI panel. It shares its mailbox-selection config, access check, and source
digest (`EmailSecurityDigest::build()`) with the `email_security_scan` job
above; the two run as independent recipes with their own
`aip_recipe_item_log` rows, so either can run on a mailbox without the
other, or both together. The job never labels, moves, or otherwise files a
message: where mail goes is the owner's decision (labels, filters), never
the model's.
...
**Verdict field**, written only by the job's `recordVerdict()`:
...
- `iem_ai_summary` (`varchar(280)`) — the one AI-authored message field this
  ...
  message body on a protected domain (see Encryption at rest, above). The
  reader's inbox list shows it as the thread's preview line (italic,
  replacing the body snippet) once a message has been summarized; a thread
  without one still shows its body snippet.

`NULL` until a pipeline recipe summarizes the message; re-running on one
message is an admin deleting the recipe's `aip_recipe_item_log` row for
...
and puts it on the recipe owner's calendar — see
