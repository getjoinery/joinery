...
# ---------------------------------------------------------------------------
fail=0
for form in preflight transfer; do
  if cmp -s "${WORK}/php_${form}.bin" "${WORK}/go_${form}.bin"; then
    echo "PASS: ${form} signing bytes are identical in PHP and Go"
  else
    echo "FAIL: ${form} signing bytes DIFFER — every Direct delivery would fail verification"
...
echo "PASS: PHP-made signatures verify in Go, over both byte-forms"
...
if PUBLIC_HTML="${PUBLIC_HTML}" WORK="${WORK}" php "${WORK}/php_verify.php"; then
  echo "PASS: Go-made signatures verify in PHP, over both byte-forms"
else
