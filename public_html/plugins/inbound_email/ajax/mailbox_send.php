 ...
 *
 * POST (multipart, for attachment uploads). Staff-only (permission >= 5); sending
 * as a mailbox is gated by the SAME MailboxViewer scope that governs reading it
 * (enforced inside MailboxSender). CSRF: the reader's persistent
 * mailbox_reader_csrf token (validated, not consumed) — the same token the
 * single-button actions use.
 *
