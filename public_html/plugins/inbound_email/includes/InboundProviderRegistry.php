 ...
 *
 * The active provider is selected by the `inbound_email_provider`
...
class InboundProviderRegistry {
    ...
    public static function all(): array {
        ...
        $plugin_dir = PathHelper::getIncludePath('plugins/inbound_email/includes/inbound_providers/');
     ...
     * inbound_email_provider setting, falling back to PostfixProvider when
     * the setting is empty or names an unknown provider.
     */
    public static function active(): string {
        $settings = Globalvars::get_instance();
        $key = trim((string)$settings->get_setting('inbound_email_provider'));
        if ($key === '') {
            $key = 'postfix';
        }
        $class = self::get($key);
