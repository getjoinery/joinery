...
plugins/joinery_ai/
  ...
  pipeline_jobs/                # Each PHP file declares one PipelineJobInterface class
    ...
    EmailTriageJob.php         # email_triage — one-line summary for one mailbox
...
- **prompt** — the system + user message text the LLM sees. In pipeline mode this is optional — blank means the selected job's `defaultPrompt()` runs, and the edit form displays that text rather than an empty box, with a **Customize** control that copies it into an editable field. Clearing the field back to empty returns the recipe to the job's own wording. Customizing stamps the built-in wording as it stood at that moment into `rcp_prompt_baseline` — the fork point. When a later release improves the built-in, the edit page compares the two and shows an upstream-changed notice with the current built-in, so the operator can merge or revert instead of drifting behind silently. Reverting clears the baseline; a prompt customized before the baseline existed has no fork point, so no notice is shown for it.
...
- **The mail-age floor.** Config `lookback_days` (default `EmailJobCandidates::DEFAULT_LOOKBACK_DAYS`, 7) bounds the pool to mail received within that many days. Every candidate costs a model call, so a recipe bound to a mailbox with an old unread backlog reads the recent slice of it, not the whole history; the floor is rolling, so mail that arrives while a recipe is paused for longer than the window is left alone too. `0` lifts the floor for a recipe that means to read everything. The same floor shapes `hasWork()` / `countWork()`, so the panel's "N to go" is the count the run will actually judge.
...
- **`email_triage`** (`plugins/joinery_ai/pipeline_jobs/EmailTriageJob.php`) — writes a one-line summary for each inbound email on the bound mailboxes, so the inbox can be scanned at a glance. Digest is `EmailSecurityDigest::build()` plus, when the message carries non-inline attachments, an appended `EmailAttachmentDigest::build()` section (metadata for every part; readable text for file-backed `text/plain` bodies and rendered `.ics` invites — see `plugins/mailbox/docs/overview.md`). The verdict is `summary` alone (one plain-language sentence, up to 280 characters). `recordVerdict()` writes `iem_ai_summary` and nothing else — where a message is filed (labels, filters) is the owner's decision, never the model's.
- **`email_schedule`** (`plugins/joinery_ai/pipeline_jobs/EmailScheduleJob.php`) — reads each inbound email on the bound mailboxes for a real, dated event (meeting, deadline, reservation, …) and puts it on the recipe owner's own calendar. Same digest as `email_triage`; when the ATTACHMENTS section carries an ICS EVENT block, the prompt directs the model to take that invite's title/start/end/timezone verbatim as the authoritative statement of the event rather than deriving them from prose. The verdict is `event_found` (bool) plus, when true, `title`/`start_local`/`end_local`/`timezone`/`all_day` and the optional details `location`/`link`/`notes` (`docs/calendar.md` § Entry details); `validateVerdict()` requires a well-formed `title` and `start_local` when `event_found` is true and rejects an `end_local` that isn't after `start_local`. **The link gate:** `recordVerdict()` keeps `link` only when it is an http(s) URL *and* appears verbatim in the digest the model was shown (`EmailScheduleJob::admittedLink()` against `EmailPipelineJobBase::digestFor()`, rebuilt from the same message) — the reminder email renders the link as something the owner clicks, so a URL the model composed, or one an email smuggled past the digest, never reaches the calendar; the entry is still proposed without it. The ICS EVENT block carries `url:` and a capped `description:` so the verbatim rule covers all three details. `recordVerdict()` does **not** configure a write target — the calendar is always the recipe owner's own, fixed in code — and resolves a missing/invalid `timezone` to the owner's profile timezone before queueing a `create_calendar_entry` **proposal** for the owner through `ActionQueue::propose()` (see [Proposed actions](#proposed-actions) and [Calendar access](#calendar-access)): the entry exists only once the owner approves the card. `source_ref` = the message id, so a log-row reset and re-run replaces the pending proposal, and an approved re-run updates the same entry instead of duplicating it. An event that has already ended (`EmailScheduleJob::endsUtc()` — the stated end, or the end of the day for an all-day entry, in the event's zone) is never proposed, whatever the model concluded: the prompt asks for `event_found = false` on past events, and this is the check that does not depend on the model having read the date. A proposal expires when its event ends, so a card nobody answered in time cannot add a past entry when it is finally opened. `event_found = false` records nothing beyond the processing-log row.
...
result, and an expiry — 7 days, or sooner when the proposer says the action
stops being answerable before that (a calendar entry's card ends with its
event). Resolved rows are kept — the queue doubles as the
...
chat composer warns before sending sensitive-looking text, whether a chat with
the Local models only add-on may use the model, and whether a domain's sealed mail may reach it.
...
| Gate | How it asks |
|---|---|
| Local models only chat | the requirement carries `trust_floor: local`, taken from the conversation's add-on (`AiConversation::localModelsOnly()`) |
...
Three AJAX endpoints back the page: `chat_send.php` (append the user message + an assistant placeholder, run the turn, finalize the placeholder), `chat_cancel.php` (stop an in-flight turn), and `chat_poll.php` (deliver a finished turn to the page). Control edits on an existing chat (toggles, model controls, privacy level, Local models only) go through the `/api/v1` `joinery_ai/chat_set_capabilities` action with the browser-session credential; queued actions list and resolve over the `/api/v1` queue actions ([Proposed actions](#proposed-actions)).
...
| | **Standard** | **Private** |
|---|---|---|
| Meaning | The server manages this chat for you | Only you can read this stored chat |
| Stored turns / title / tool traces / attachments sealed at rest | — | ✓ |
| Unlock required to read or continue | — | ✓ (in the vault window) |
| Inference provider | any (cloud or local) | any (operator's choice), or local only with the add-on below |

Private carries one add-on, shown under **Extra protection** in the chat's settings panel:

| Add-on | Protects | Costs |
|---|---|---|
| **Local models only** (`aic_local_models_only`) | Nothing in this chat is sent to an outside AI company. | Only AI models running on your own hardware can answer. |

The add-on is offered when a local model is configured (`ChatLevel::localOnlyAvailable`). It is **one-way**: once on, it stays on, and a request to switch it off is refused. It takes effect only on a Private chat — a chat lowered to Standard keeps the stored flag, inert, and raising it to Private again restores the add-on (so that raise needs a configured local model and re-pins the model). `AiConversation::localModelsOnly()` is the one answer to "is the add-on in effect?". The chat list and thread payloads (`ChatSerializer::conversationSummary`) carry `local_models_only` beside `security_level`, and the settings summary line shows it.

The unit is the **conversation** (`aic_conversations`); there is no grouping above it. A new chat takes the plugin-wide defaults (`joinery_ai_default_chat_level` and `joinery_ai_default_chat_local_only`, set on the Joinery AI settings page) unless the composer's Privacy control and add-on switch override them — a choice whose prerequisites are missing (Private needs a set-up vault; Local models only needs a Private chat and a configured local model) is dropped so a chat never claims a protection it can't deliver. `ChatLevel` owns those prerequisites and the resolution.
...
**Extraction is isolated, and it is the platform's.** Reading a file's words is [`DocumentText`](../../../docs/document_text.md), the core extractor: parsing runs only in a short-lived `timeout N php -d memory_limit=…` subprocess, so a bomb or a hang kills only the child and the parent marks the attachment un-extractable and continues. Cloud-stored bytes go down that subprocess's stdin rather than through a temp file, so decrypted content never lands on disk. `AiAttachment` keeps the policy half — which types this plugin accepts, the per-category caps, and how an outcome routes into the model payload; the core extractor's `secured` and `too_large` outcomes collapse to `failed` here, because for send-time routing an encrypted PDF is simply one with no readable text. Text is capped before it reaches the model. SVG, archives, and any other type outside `AiAttachment::CATEGORY` are rejected at ingress, whatever the core extractor could read.

**A document's name can open a door, but never forge one.** Office and OpenDocument files ARE zips, and libmagic tells them apart by convention rather than by rule — so on some builds a perfectly good `.docx` arrives detected as `application/zip`. `resolveUploadMime()` consults the filename in exactly that case: the detected type landed on a bare container, and the name claims a format built on one. The fallback cannot reach `image` or `pdf`, the two categories that send raw bytes to a model, so a lying name can never forge a block — it only chooses which text extractor gets a look. And the extractor has the last word: it opens the bytes inside its sandbox and reports what they really were, so a plain zip renamed `.docx` is refused after the fact with a message saying the file is not what its name says.
...
Caps are settings-tunable: `joinery_ai_attach_image_max_bytes` (5 MB), `joinery_ai_attach_pdf_max_bytes` (10 MB), `joinery_ai_attach_document_max_bytes` (10 MB), `joinery_ai_attach_text_max_bytes` (2 MB), `joinery_ai_attach_max_per_message` (5).
...
- **Writes never go through `$ai_writable_fields`** — `CalendarEntry::authenticate_write()` lets any permission-≥5 caller write any subject's entry (recipes are typically admin-configured), so a raw write tool would let model output aim at another user's calendar. Every AI-originated entry instead goes through one shared owner-fixed helper, `CalendarEntryImporter::upsert()` (`includes/calendar/CalendarEntryImporter.php`, core): it fixes the subject from the *caller's* code, derives UTC from wall-clock + timezone, always writes `cal_status = 'tentative'`, and dedupes on `(cal_source, cal_source_event_id)` scoped to the owner so a re-run updates the same entry instead of duplicating it.
  - **Agent mode** (chat, agent recipes) enters through the `create_calendar_entry` action (`logic/create_calendar_entry_logic.php`, `ai_agent: 'confirm'`) — the acting user is always the session user; the model supplies title/time/timezone and, optionally, location/link/notes (the importer refuses a link that is not http(s)). The approval card shows the location, the link in full (`ProposedActionFacts::verbatim`), and a bounded notes line.
...
Extraction is pure string/DOM work on already-downloaded bytes, and it runs in the parser jail: the tool downloads in the pool — the SSRF guard (the platform-shared `includes/UrlSafetyValidator.php`, restricting `fetch_url` to ports 80/443), IP pinning, redirect re-validation, and size/time caps all run there — and hands the body to `FetchUrlReader`, a [sandbox parser](../../../docs/document_text.md#sandbox-parsers) that does the DOM walk, the escalation and the flatten as the `joinery-jail` user with no network, no config and no key. The HTML is parsed with `LIBXML_NONET` so the parser can never fetch an external DTD or entity. **Reader mode is a structure filter, not a trust filter:** its output is exactly as untrusted as the raw page and still flows through the chat assistant's untrusted-content handling. Non-HTML responses (JSON, plain text, CSV) bypass extraction in both modes.
