...
4. The page polls `chat_poll.php?message_id=N` (owner-scoped) every ~0.6s. While `running` it gets `partial_text` plus `activity` (the runner's live stage label — "Waiting for {model}…", "Running tool: {name}…", "Writing…") and `running_seconds` (server-computed elapsed time), rendering them as a status line under the streaming text; the partial text shows in a live bubble as plain text; on `complete` it swaps in the final markdown bubble via `ChatRender::assistantBubble`; on `failed` it shows the error. `chat_confirm` resumes the same way (seeding the sink with the pending lead text), finalizing the pending row in place.
