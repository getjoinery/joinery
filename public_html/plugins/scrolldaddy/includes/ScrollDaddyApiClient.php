...
class ScrollDaddyApiClient {
	...
	public static function callPrimary($path, $method = 'GET', $timeout = 5) {
		...
		$url = $settings->get_setting('scrolldaddy_dns_internal_url');
	...
	public static function callSecondary($path, $method = 'GET', $timeout = 5) {
		$settings = Globalvars::get_instance();
		$url = $settings->get_setting('scrolldaddy_dns_secondary_internal_url');
		$api_key = $settings->get_setting('scrolldaddy_dns_secondary_api_key');
		if (!$url) return null;
