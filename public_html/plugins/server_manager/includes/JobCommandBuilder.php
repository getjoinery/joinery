...
BASH;
	...
	public static function build_provision_relay($node, $params) {
		...
		//    steady-state ssh/rsync consumers connect with it, not the node key.
		$pull_pub_esc = escapeshellarg($pull_pubkey);
		$steps[] = ['type' => 'ssh', 'label' => 'Authorize relay pull key', 'on_host' => true,
			'cmd' => "sudo mkdir -p /root/.ssh && sudo chmod 700 /root/.ssh && "
			       . "( sudo grep -qxF {$pull_pub_esc} /root/.ssh/authorized_keys 2>/dev/null || "
			       . "echo {$pull_pub_esc} | sudo tee -a /root/.ssh/authorized_keys >/dev/null ) && "
			       . "sudo chmod 600 /root/.ssh/authorized_keys && echo PULL_KEY_AUTHORIZED"];

		// 5. Peer the main box's WireGuard key so Joinery can dial out (idempotent).
		if ($main_wg_pubkey !== '') {
		...
		// 6. Verify + emit the markers the result processor parses.
