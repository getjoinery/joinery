 ...
 *                host's own agent (mgh_mgn_managed_node_id), so host-scope routing has a target
 * @version 1.1 - ensure_for_node(): the placement record is minted (or linked) the moment a
 *                container node needs one, so mgn_mgh_managed_host_id is the only sibling identity
 */
...
class ManagedHost extends SystemBase {
	...
	public static $pkey_column = 'mgh_managed_host_id';

	protected static $foreign_key_actions = [
		'mgh_mgn_managed_node_id' => ['action' => 'null'],
	];

	public static $field_specifications = array(
		'mgh_managed_host_id'                   => array('type'=>'int8', 'is_nullable'=>false, 'serial'=>true),
		...
		'mgh_mgn_managed_node_id'     => array('type'=>'int8'),
	...
	public function host_node() {
		$node_id = (int)$this->get('mgh_mgn_managed_node_id');
	...
	public static function placement_for_addresses(array $addresses) {
		...
		foreach (new MultiManagedHost(['deleted' => false], ['mgh_managed_host_id' => 'ASC']) as $host) {
	...
	public static function ensure_for_node($node) {
		...
		$existing = new MultiManagedHost(['deleted' => false], ['mgh_managed_host_id' => 'ASC']);
		$existing->load();
		$host = null;
		foreach ($existing as $ex) {
		...
		if ((int)$node->get('mgn_mgh_managed_host_id') !== (int)$host->key) {
	 ...
	 * mgh_mgn_managed_node_id. A host agent that pairs but is never named there is
	...
	public static function link_host_node($node) {
		...
		$candidates = new MultiManagedHost(['host' => $addr, 'deleted' => false], ['mgh_managed_host_id' => 'ASC']);
		foreach ($candidates as $host) {
			...
			$host->set('mgh_mgn_managed_node_id', (int)$node->key);
			$host->save();
			return $host;
		}
		return null;
	...
	public static function pick_for_provisioning() {
		$db = DbConnector::get_instance()->get_db_link();
		$q = $db->query(
			"SELECT mgh.mgh_managed_host_id " .
			"FROM mgh_managed_hosts mgh " .
			"LEFT JOIN mgn_managed_nodes mgn ON mgn.mgn_mgh_managed_host_id = mgh.mgh_managed_host_id AND mgn.mgn_delete_time IS NULL " .
			"WHERE mgh.mgh_provisioning_enabled = true AND mgh.mgh_delete_time IS NULL " .
			"GROUP BY mgh.mgh_managed_host_id " .
			"HAVING COUNT(mgn.mgn_managed_node_id) < mgh.mgh_max_sites " .
			"ORDER BY COUNT(mgn.mgn_managed_node_id) ASC, mgh.mgh_managed_host_id ASC " .
			"LIMIT 1"
		);
		$row = $q->fetch(PDO::FETCH_ASSOC);
		if (!$row) return null;
		return new ManagedHost($row['mgh_managed_host_id'], true);
	}
}
