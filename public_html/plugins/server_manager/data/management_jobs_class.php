...
class ManagementJob extends SystemBase {
	...
	const LOG_EXCERPT_TYPES = array('site_log', 'log_table_tail', 'unit_journal', 'file_head');

	public static $field_specifications = array(
		'mjb_management_job_id'                => array('type'=>'int8', 'is_nullable'=>false, 'serial'=>true),
		'mjb_mgn_managed_node_id'      => array('type'=>'int8'),
	...
	function rerun($created_by) {
		...
		$node_id  = $this->get('mjb_mgn_managed_node_id');
	...
	static function requeueStaleClaims($node_id = null) {
		...
		$args = [$cutoff];
		if ($node_id !== null) {
			$sql .= ' AND mjb_mgn_managed_node_id = ?';
			$args[] = (int)$node_id;
		}
	 ...
	 *
	 * Replaces the copy-pasted "SELECT mjb_management_job_id ... ORDER BY mjb_management_job_id DESC LIMIT 1"
...
class MultiManagementJob extends SystemMultiBase {
	...
	protected function getMultiResults($only_count = false, $debug = false) {
		...
		if (isset($this->options['node_id'])) {
			$filters['mjb_mgn_managed_node_id'] = [$this->options['node_id'], PDO::PARAM_INT];
		}
