...
Everything the words return is redacted **on the node** by the agent's `redact` package before it leaves: credential values by the key names `SmSecretRedactor` masks (the two lists are pinned equal by `tests/unit/agent_redactor_parity_test.php`), the local part of email addresses (the domain is kept, since a mail diagnosis needs it), IP addresses, and long opaque tokens. The plane's `SmSecretRedactor` is the second pass, at display time, as for every transcript. Member addresses, submitted forms and raw payloads are not returned at all: the table word reads a compiled column list that leaves them out.
