...
A recipe has a **scope**. `fail2ban` is host-scoped: its subject is the machine's units and jails. An agent inside a container cannot see that machine, so the loop does not tick a host-scoped recipe there: one journal line and one ledger line (`not_applicable`) at start, and the claim reports the recipe as `fail2ban:not-applicable`, which the Host card explains. The host's own agent watches the host.
...
**Report-only** is a compiled constant in the agent (`recipes.ReportOnly`). While it is on, an attempt is ledgered with the outcome `report-only` and runs nothing; the two-tick rule, the backoff, the budget and the escalation count exactly as if it had run, so the ledger shows the path the armed loop would take. Arming is a release.

The recipes an agent runs, each with its mode, travel in every claim (`recipes`: `name:report-only` or `name:armed`, comma-separated), are normalised on intake like the vocabulary (`AgentChannelEndpoint::normalised_recipes`) and stored in `mgn_agent_recipes`. The Host card on the node page opens with that line, so a person can see that a node is report-only. A report-only escalation opens a case just the same; the burn-in's product is those cases.
