...
Two recipes are compiled in. **`fail2ban`**: the unit is active with at least one jail, or `host_converge` runs `host_housekeeping.sh`. **`agent_supervision`**: something would restart the agent if it stopped (`agent_report`: systemd supervising this process with a restarting unit, or the cron keepalive installed and switched on), or `agent_converge` runs `install_agent.sh`, which writes the supervision and, under the job marker an attempt holds, restarts nothing. The host timer's daily converge already ran that installer, so a lost unit file or keepalive was back within a day; the recipe makes it minutes. An agent that a person started by hand on a systemd box is genuinely unsupervised and the installer cannot fix that without restarting it, so three attempts open a case and a person restarts the agent.
