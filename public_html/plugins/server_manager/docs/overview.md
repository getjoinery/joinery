...
Two ledgers, both JSON lines, one per recipe: `/etc/joinery-agent/ledger/<recipe>.jsonl` is root-only and is the one the loop reads back (attempts in the last hour, the open escalation); an attempt is written when it starts and its outcome when it ends, so a repair that kills the agent is still a counted, failed attempt on the next start. `{site}/cache/recipes/<recipe>.jsonl` is the same lines, readable by the site, written and never read. Events: `check`, `attempt`, `outcome` (`repaired`, `failed`, `report-only`, `interrupted`), `held`, `hold_ignored`, `busy`, `escalation`, `escalation_note`, `escalation_closed`.
...
The **Cases** card on the node page (`IncidentCaseCard`, under the Host card) lists a node's cases open first, then the newest closed, each with what the node tried, the note a human wrote and a mark-read control; both writes are POST actions (`case_note`, `case_read`). Neither closes the case. Every field is escaped and nothing in a case is a link. Two admin-header notices come from the same facts on every admin page of the management node (`FleetAttentionNotice`, registered from the plugin bootstrap): a node whose latest host report names a failed unit, and an open case nobody has marked read.

On the node itself the agent also writes a rendered copy of the case outward, `{site}/cache/recipes/<recipe>.case.json` beside the outward ledger, written whole and never read back, carrying the case, its body and its delivery (`local`, or `management node` when a plane is polling). The site's admin header shows an open one as a core notice (`RecipeCaseNotice`) worded "as reported by the agent's ledger", on a paired node too (saying the management node has it). The hourly `RecipeCaseMail` task is the unpaired path only: while a case whose delivery is `local` is open it sends the superadmins one plain-text mail per recipe per day — no markup, no link, the last send per recipe kept in the `recipe_case_mail_log` setting — and a case a management node has is never mailed from the node, because the plane's card and notice already carry it. The web user can forge that file; what a forgery buys is a lie to the admin and one mail, and nothing root acts on.
