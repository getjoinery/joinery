...
Two ledgers, both JSON lines, one per recipe: `/etc/joinery-agent/ledger/<recipe>.jsonl` is root-only and is the one the loop reads back (attempts in the last hour, the open escalation); an attempt is written when it starts and its outcome when it ends, so a repair that kills the agent is still a counted, failed attempt on the next start. `{site}/cache/recipes/<recipe>.jsonl` is the same lines, readable by the site, written and never read. Events: `check`, `attempt`, `outcome` (`repaired`, `failed`, `report-only`, `interrupted`), `held`, `hold_ignored`, `busy`, `escalation`, `escalation_note`, `escalation_closed`. A `check` line is written on every failing tick and whenever the verdict changes; a pass after a pass, or an unknown after an unknown, writes nothing, so a healthy recipe's ledger is its restarts and a recipe that cannot answer does not fill its ledger with the same answer.
