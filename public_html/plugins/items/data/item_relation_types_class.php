...
class ItemRelationType extends SystemBase {
	...
	public static $permanent_delete_actions = array(
		'itt_item_relation_type_id' => 'delete',	
...
class MultiItemRelationType extends SystemMultiBase {
	...
	function _get_results($only_count=FALSE, $debug = false) {
		...
		if ($only_count) {
			$sql = 'SELECT COUNT(1) as count_all FROM itt_item_relation_types
				' . $where_clause;
		} else {
			$sql = 'SELECT * FROM itt_item_relation_types
				' . $where_clause . '
				ORDER BY itt_item_relation_type_id ASC' . $this->generate_limit_and_offset();
		}
	...
	function load($debug = false) {
		...
		foreach($q->fetchAll() as $row) {
			$child = new EventType($row->itt_item_relation_type_id);
