<?php
	...
	if ($_POST) {
		...
		$product_group = new ProductGroup($_POST['prg_product_group_id'], TRUE);
		...
		LibraryFunctions::redirect('/admin/admin_product_group_edit?p=' . $product_group->key);	
		exit;
	} 
	...
	if (isset($_GET['p'])) {
		...
		$product_group = new ProductGroup($_GET['p'], TRUE);
		...
		$headers = array('Product Group ID', 'Product Group Name');
		$altlinks = array();
		$box_vars =	array(
			'altlinks' => $altlinks,
		...
		foreach($product_groups as $product_group) {
			$rowvalues=array();
			array_push($rowvalues, $product_group->get('prg_product_group_id'));
