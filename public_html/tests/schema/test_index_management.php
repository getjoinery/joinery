...
try {
    ...
    echo "\n[Phase 1] create plain field-level indexes\n";
    ...
    check("manageIndexes succeeded", $r1['success'] === true);
    check("single-column index created ($single)", in_array($single, $idx));
    check("composite index created ($composite_exact)", in_array($composite_exact, $idx));
    check("added count is 2", count($r1['indexes_added']) === 2);

    // Idempotency
    $r1b = $updater->manageIndexes([IdxStubPhase1::class]);
    check("second run adds nothing (idempotent)", count($r1b['indexes_added']) === 0);

    // Hand-made + PK survive
    check("hand-made non-_idx index still present", in_array('zz_idx_test_handmade', table_indexes($dblink, $TEST_TABLE)));

    // ---------------- Dedupe: equivalent index under another name ----------------
    echo "\n[Dedupe] equivalent btree under a different name is not duplicated\n";
    ...
    check("no recreate when equivalent exists under legacy name", !in_array($single, $rd['indexes_added']));
    ...
    echo "\n[Phase 2] obsolete index dropped under cleanup\n";
    $r2 = $updater->manageIndexes([IdxStubPhase2::class]); // composite no longer declared
    check("composite index dropped", in_array($composite_exact, $r2['indexes_removed']));
    check("composite gone from pg_indexes", !in_array($composite_exact, table_indexes($dblink, $TEST_TABLE)));
    check("single index retained", in_array($single, table_indexes($dblink, $TEST_TABLE)));
    check("hand-made survives cleanup", in_array('zz_idx_test_handmade', table_indexes($dblink, $TEST_TABLE)));
    check("PK index survives cleanup", in_array("{$TEST_TABLE}_pkey", table_indexes($dblink, $TEST_TABLE)));
    ...
    check("upgrade-only does not drop obsolete", count($ru['indexes_removed']) === 0
          && in_array($composite_exact, table_indexes($dblink, $TEST_TABLE)));

    // ---------------- Phase 3: advanced indexes ----------------
    echo "\n[Phase 3] partial, expression, and partial-unique indexes\n";
    ...
    check("partial index created (discriminator-named)", count($partial) >= 1);
    check("expression index created (hash-named)", count($expr) >= 1);
    check("partial-unique index created (_uidx)", count($uidx) >= 1);

    // Partial-unique semantics: duplicate among ACTIVE rows is rejected,
    // but allowed when one of the pair is soft-deleted.
    echo "\n[Phase 3] partial-unique duplicate handling\n";
    ...
    check("partial-unique tolerates soft-deleted duplicate (still present)",
    ...
    check("active-row duplicate skips unique index with warning", $warned);
    check("no _uidx created while active duplicates exist",
