...
try {
	...
	check('fixture: private file created', $file && $file->key);

	// Sanity: unsigned anonymous request is denied (404, existing behavior).
	$plain_url = $file->get_url();
	$r = http_get($plain_url);
	check('unsigned request without session is denied', $r['status'] == 404, 'status ' . $r['status']);

	// ---- Valid signature serves with no session --------------------------
	$signed = $file->mintSignedUrl('original', 120);
	$r = http_get($signed);
	check('signed URL serves without session', $r['status'] == 200, 'status ' . $r['status']);
	check('signed response body matches file bytes', $r['body'] === $bytes);
	check('signed response is private, no-store',
	...
	$r = http_get($tampered);
	check('tampered signature is denied', $r['status'] == 404, 'status ' . $r['status']);
	...
	$r = http_get($extended);
	check('extended expiry is denied', $r['status'] == 404, 'status ' . $r['status']);

	// ---- Expired signature falls back to the gate -------------------------
	$short = $file->mintSignedUrl('original', 1);
	sleep(2);
	$r = http_get($short);
	check('expired signature is denied', $r['status'] == 404, 'status ' . $r['status']);
	...
	$r = http_get($swapped);
	check('size-swapped signature is denied', $r['status'] == 404, 'status ' . $r['status']);
	...
	check('owner passes is_viewable regardless of signatures', $file->is_viewable($owner_session) == true);

	// ---- Cloud-private never-302 note --------------------------------------
	if ($file->get('fil_storage_driver') === 'cloud') {
		$r = http_get($file->mintSignedUrl('original', 120));
		check('cloud-private signed response streams (no redirect)', $r['status'] == 200);
	} else {
