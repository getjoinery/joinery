...
- **A remote job source.** In node posture the agent polls its control plane **outbound over HTTPS** with a per-node **pairing credential** minted at enrollment (the plane stores only a verifier). Claim, execute, post results — the same lifecycle `mjb_management_jobs` already models, exposed through plane API endpoints. Polling gives the plane agent-liveness for free (last-poll time = the heartbeat).
...
- **Provisioned installs** (`install_node`, Linode StackScript): the provisioning key is used to install the platform + agent, the node pairs with its plane during install, and the final install step **removes the key's `authorized_keys` entry and stops sshd** (A5 — unless the host has no rescue path). Nothing standing survives birth.
- **Self-installs** (customer runs the installer): the installer already runs on-box as root; it installs the agent and performs pairing (the sentinel spec's §12.2 enrollment is this flow's customer-facing form). No plane SSH ever exists.
...
**Phase 1 — the channel.** Agent: remote job source, pairing, primitive dispatch + validation, acceptance policy, manifest verification for script-invoking primitives. Plane: claim/result API endpoints, pairing issuance, job routing (primitive jobs to paired agents; SSH otherwise). Ships as an ordinary signed agent release — the fleet self-updates into capability.
...
- **A. Agent: remote job source + pairing** · *medium* — HTTPS polling mode, claim/backoff, result posting, pairing credential storage (root-owned, node-side).
...
- **D. Plane: channel endpoints + routing** · *small–medium* — pairing issuance/verifier, claim/result API, `JobCommandBuilder` routing (`build_<op>_primitive` joins `_api`/`_ssh` in `transports_for()`), per-node cutover flag.
