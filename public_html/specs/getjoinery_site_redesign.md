...
**Status:** **BUILT 2026-08-09 — structure and copy are live on getjoinery.com.** Every §9 decision was resolved first (§9.2/§9.4 earlier; §9.1, §9.3, §9.5–§9.8 by the owner on 2026-08-09, each recorded in place in §9). The pages of §4–§5 ship as **CMS pages built from component blocks** (§10b), the §7 redirects are rows in the URL manager, the navigation is rows in the public menu table, and `/page/apps`, `/page/install`, `/page/why`, `/page/leave-gmail`, `/page/nextcloud-alternative`, `/page/families` are new. Launch framing: a **beta**, with screenshots as the proof device (demo deferred to `specs/getjoinery_demo_instance.md`). Governing copy constraint from §9.8 is enforced throughout: **no unbuilt protocol or client names anywhere on the site** — CalDAV and IMAP appear only as honest "not yet" answers. Companion spec `specs/automatic_install_mail_topology.md` has its own short open list.

**What remains before this can be called launched** (none of it is copy work): the screenshot debt in §10, the store SKUs the pricing CTAs want (§10a below), and the developer-site content divergence (§6). The spec stays active until those close.
**Updated 2026-08-09** to reflect platform work landed since the draft: calendar reminder/summary emails shipped (§9.8), Drive Private tier + encrypted sync + soak-rig hardening (§5.3), the AI sealed-content guard (§5.3), Ubuntu 24.04/26.04 install targets (§5.2), fleet backups with write-only node keys (§5.4), and Fortress/sending-identity hardening (§9.3) (the level then called Fortress; now Private with the mail add-ons Seal at the relay and Only send while I'm signed in). Copy claims below now cite the shipped mechanisms.
...
4. **Founder license — $499, limited to 200.** Lifetime business license: one production install, lifetime updates, the whole bundled free suite licensed for business use (mail, events, bookings, messenger, items, DNS filtering, the AI assistant, vault — every bundled plugin and theme that is not sold separately), and **every first-party paid plugin free forever** — the current two ($248 of plugins today) and everything published later. The pitch is stated honestly: founders are betting the plugin catalog grows, and the $100 premium over the standard license buys the whole catalog for life — already rational on day one against today's two plugins alone. Show a **real live counter** ("N of 200 remaining") — genuine scarcity displayed plainly, never a countdown timer or invented deadline.
5. **Standard business license — $399 lifetime.** Opens when the founder cohort sells out. One production install, lifetime updates, the same bundled free suite licensed for business use, and the *paid* plugins sold separately. Occasional sales. Dev/staging copies free (standard convention, stated). The only thing the Founder premium buys over this is the paid-plugin catalog for life — the free suite is never the differentiator, because "no feature limits" has to be literally true in the licence a buyer receives.
...
## 10a. How it was actually built (2026-08-09)

**Pages.** Six new theme views (`apps`, `install`, `why`, `leave-gmail`, `nextcloud-alternative`, `families`) alongside the rewritten `index`, `pricing` and `about`. Each is the standard three-line view that renders one `gj-*` component; SEO title and description are passed to `public_header()` rather than emitted in the view, per `docs/seo_metadata.md`.

**Redirects are theme views, not routes.** `/features`, `/philosophy`, `/showcase`, `/developers`, `/documentation` and `/license` are 301 stubs in `theme/getjoinery/views/`. This keeps a marketing-site concern inside the marketing theme instead of in core `serve.php`, which every deployment shares. Two helpers on the theme's `PublicPage` do the work: `redirect_permanent()` for same-site moves and `redirect_to_developers()` for the ones that left. **The second one no-ops when the request is already on the developer site**, which runs this same theme — without that guard `developers.getjoinery.com/documentation` would redirect to itself forever. `documentation` and `license` fall through to the core view after the guard, so the developer site renders them.

**Data-driven blanks.** Copy is static HTML in the database, which is wrong for anything that must be true — where a buy button points, and how many founder licences remain. Those are `{{TOKENS}}` resolved per request by `theme/getjoinery/includes/MarketingTokens.php` against real product records. A product that does not exist degrades to a working enquiry link, never a 404; the founder counter renders only from real stock and is otherwise omitted entirely. Unknown tokens are stripped rather than shown.

**Seeder gained modes.** `utils/seed_getjoinery_content.php` now takes `--check` (report drift, change nothing), `--update` (overwrite), and `--update --only=slug,slug`. The `--only` filter earned its place immediately: `--check` against production revealed that `gj-developers` and `gj-showcase` had been hand-edited live, and a bare `--update` would have silently discarded them.

**New CSS** in the theme stylesheet (v1.3.0) for the shapes the spec needed and the theme lacked: the install ladder, the app-vs-incumbent map, the FAQ accordion, the numbered migration checklist, ownership tiles, pricing lanes and the founder counter. `.pricing-teaser` also gained styles — it had been referenced by the old homepage and never styled at all.

### 10b. Second pass: pages moved out of the theme and into the CMS (2026-08-09)

The first pass put the copy in the database but kept a view file per page in the theme, plus a token resolver and a hardcoded nav. That still left this site's content inside a theme, so a second pass moved the lot into ordinary CMS records. **The theme now contains a stylesheet, block templates, page chrome and one generic page template, and knows none of this site's words.**

**Pages are CMS pages.** Eleven of them — the nine marketing pages plus terms and privacy — each a `pag_pages` row with an ordered `pag_component_layout` of component blocks, editable in the admin page editor like any other page. `utils/getjoinery_pages.php` holds the definitions and `utils/seed_getjoinery_content.php` applies them (`--check` / `--update` / `--only`, same contract as before, now per page and per block).

**Blocks, not markup.** Composed from the theme's existing blocks (`marketing_hero`, `trust_badges`, `audience_grid`, `comparison_cards`, `pricing_teaser`, `cta_section`) plus nine new ones written for the shapes the spec needed: `app_map`, `prose`, `option_ladder`, `comparison_table`, `checklist`, `tile_row`, `faq`, `offer_cards`, `commitment_list`. Only terms and privacy use `custom_html`, which is the right tool for a page that is one long legal document.

**The pages live at `/page/{slug}`** — the CMS's own addressing, used as-is. An earlier attempt at this added a routing fallback so a page could answer at `/{slug}`, along with a canonical-URL option and header/footer locations on the public menu. All of that was **reverted**: the site is under construction, `/page/` URLs are fine, and none of it justified new surface in core. The one platform change that stayed is a bug fix (below).

**Redirects and navigation are data.** The redirects are `url_urls` rows (`utils/seed_getjoinery_redirects.php`); the header nav is `pmu_public_menus` rows (`utils/seed_getjoinery_navigation.php`), which needs no core change because `get_menu_data()['main_menu']` already existed. Being per-deployment is the point: the developer site simply has no redirect rows, which is what stops it redirecting to itself — the host-sniffing guard the first pass needed is gone. The **footer** link list is the theme's own markup by decision: the header is editable site data, the footer is a design choice.

**One bug fixed in core:** `alternate_homepage` pointing at `/page/{slug}` returned a 404, because serve.php pre-loaded the page but never put the slug in `$params`, and `views/page.php` resolves the page itself from `$params`. Reproduced with and without the fix. This is what lets the homepage be a CMS page.

**The token resolver is gone.** Button URLs are ordinary fields on the block, editable in the admin — which is what an editor would expect and where a store URL should be set once the products exist.

**A second core bug turned up while deploying** and is fixed alongside: `migrations/recipe_mailbox_alias_to_list.php` assumed a joinery_ai column exists. That plugin is installed but inactive on getjoinery.com, so its tables never sync and sit at an older schema without the column — the migration threw and rolled the whole upgrade back, twice. A core migration cannot assume a plugin's columns exist.
...
5. **The developer site still serves the marketing clone.** §6's content divergence is untouched by this build.
6. **Business-lane copy must state the suite is included.** The licence files now scope a business purchase to core plus every bundled plugin and theme that is not sold separately (`LICENSE-BUSINESS.md` § Definitions), which is what makes the pricing page's "no feature limits" claim true. The Founder and Standard cards on `/pricing` are page content in the admin editor, so they need that sentence added by hand — and the Founder card's differentiator reworded to the *paid* plugin catalog, since the free suite now comes with either licence.
