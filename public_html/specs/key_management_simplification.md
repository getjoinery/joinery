...
| 4 | **Escrow blob missing**: `replicateBlob()` is best-effort (failure only `error_log`s); if the management node dies and the replica never landed, the node key is gone with it | Real until envelope model retires blobs | Interim: make replication failure fail the backup job loudly (Phase A); envelope model removes the side-channel entirely (sealed keys travel inside the artifact) |
