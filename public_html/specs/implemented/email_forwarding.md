...
├── plugin.json
├── uninstall.php
├── setup_email_forwarding.sh        ← dynamically generated by admin page
├── data/
...
- Updated `/docs/email_system.md` — Added note and link to email forwarding doc
- Updated `CLAUDE.md` — Added to documentation index

**Note:** Postfix and opendkim installation are NOT added to `install.sh` or `Dockerfile.template`. Since this is an optional plugin, server-level dependencies are handled by the plugin's own `setup_email_forwarding.sh` script, which is dynamically generated by the domain admin page based on the current server state. This avoids installing unnecessary SMTP infrastructure on sites that don't use email forwarding.
...
### Server Setup via Plugin Script

Instead of modifying `install.sh` or `Dockerfile.template` (which would install Postfix/opendkim on every Joinery site regardless of whether the plugin is used), server-level dependencies are managed by the plugin itself.

The domain admin page (`admin_email_forwarding_domains.php`) dynamically generates `setup_email_forwarding.sh` based on current server state. It detects what's missing and builds a tailored script that includes only what's needed:

```bash
# Example generated script (contents vary based on server state)
# Install Postfix (if not installed)
debconf-set-selections <<< "postfix postfix/mailname string $(hostname -f)"
debconf-set-selections <<< "postfix postfix/main_mailer_type string 'Internet Site'"
apt install -y postfix

# Configure Postfix transport (if not configured)
postconf -e "virtual_transport = joinery"
postconf -e "virtual_mailbox_domains = example.com"
# ... master.cf pipe transport entry

# Install opendkim (if not installed)
apt install -y opendkim opendkim-tools

# Generate DKIM keys per domain (if missing)
mkdir -p /etc/opendkim/keys/example.com
opendkim-genkey -s mail -d example.com -D /etc/opendkim/keys/example.com
```

The admin page also opens UFW port 25 via the script, and handles RBL configuration for Postfix.
...
### Plugin Setup
- [x] Plugin directory structure at `/plugins/email_forwarding/`
- [x] `plugin.json` metadata (v1.1.0)
- [x] `uninstall.php` cleanup script (includes sequence cleanup)

### Data Layer
- [x] Data models (domain, alias, log) in `/plugins/email_forwarding/data/`
- [x] Plugin migration (`/plugins/email_forwarding/migrations/migrations.php`) for settings and admin menu entry

### Core Processing
- [x] EmailForwarder class with manual header/MIME parsing (~683 lines)
- [x] SRSRewriter class (encode, decode, validate — ~145 lines)
- [x] SRS bounce handling in EmailForwarder (detect SRS recipient, decode, forward bounce)
- [x] DKIM verification on inbound (verify signature, log failures, pass unsigned)
- [x] Postfix pipe script at `/plugins/email_forwarding/scripts/`
- [x] Raw forwarding via SmtpMailer (preserves MIME, attachments, no template wrapping)
- [x] Rate limiting via forwarding log table (per-alias and per-domain)
- [x] Basic header checks (missing From, oversized messages)

### Admin Interface
- [x] Admin pages (aliases, domains, logs) with tab navigation
- [x] Admin menu entry under Emails → Incoming
- [x] DNS/Postfix validation panel on domain page
- [x] Dynamic `setup_email_forwarding.sh` generation based on server state

### Server Configuration
- [x] Postfix/opendkim setup handled by plugin's dynamically generated `setup_email_forwarding.sh` (not install.sh/Dockerfile — these are plugin-specific dependencies, not base platform)
- [x] Postfix RBL configuration included in generated setup script
- [x] Postfix setup documentation (bare-metal and Docker multi-container)
- [x] Server status detection panel (Postfix, transport, opendkim)
- [x] Stale Postfix config detection (active domains not in Postfix, deleted domains still in Postfix)
- [x] mydestination conflict detection and warning

### Documentation & Maintenance
- [x] `/docs/email_forwarding_plugin.md` — setup guide, admin usage, server config, DNS, troubleshooting, local testing
- [x] Updated `/docs/email_system.md` with note and link to forwarding doc
- [x] Updated `CLAUDE.md` documentation index
- [x] Scheduled task for forwarding log cleanup
...
- [x] Admin can add a forwarding domain and create aliases via the admin UI
- [x] Email sent to an alias is forwarded to all configured destinations
- [x] Email to a non-existent alias is rejected (or caught by catch-all)
- [x] Forwarded email preserves original From (via display name), Subject, and body content
- [x] All forwarding transactions are logged with status
- [x] Disabled aliases/domains stop forwarding immediately
- [x] Rate limiting prevents runaway forwarding
- [x] SRS rewrites envelope sender so forwarded mail passes SPF at destination
- [x] SRS bounces are decoded and forwarded to the original sender
