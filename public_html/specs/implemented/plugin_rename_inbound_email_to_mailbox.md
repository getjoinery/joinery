...
|---|---|---|---|
| 1 | `plugins/mailbox/` | `plugins/mailbox/` | Covers `PathHelper::getIncludePath(...)`, requires, script paths, doc links. |
...
- `plugins/mailbox/includes/InboundEmailSetupCheck.php` (~line 895): the pipe-path
  suffix string `plugins/mailbox/utils/inbound_email_handler.php` →
...
3. **CLAUDE.md and GEMINI.md are NOT edited on disk.** They are generated from the
   ...
   the docs-index line (`plugins/mailbox/docs/overview.md` →
...
3. Re-render the Postfix pipe config — `master.cf` still points at
   `plugins/mailbox/utils/inbound_email_handler.php`, so **live inbound mail
