...
|-------|----------|----------|
| Site domain | **Yes** | *Amended 2026-08-11 (was: optional, blank means the instance's IP).* A site with no domain can get no certificate, and every canonical URL and link it emits names an IP address; moving to a real domain afterwards is reconfiguration rather than deployment. Running on a bare IP is a state worth passing through during setup and not one worth deploying into, so the form no longer offers it. The label also says what the field is for, because an empty box beside "Site domain" reads as optional — a deployer who assumed it was required typed a placeholder that will never resolve, which then named the site directory, set `webDir`, and armed a retry timer for a hostname nobody owns. `install.sh` keeps its IP auto-detection: it is still reached by a hand-run install, and this is a Linode-form decision, not a platform one. |
