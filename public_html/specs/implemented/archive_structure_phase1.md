# Specification: Archive Structure Changes - Phase 1
...
- Include maintenance scripts in the archive (see list below)
...
**Files to include in maintenance_scripts/ directory:**
```
maintenance_scripts/
...
$required_files = [
    $maintenance_source . 'server_setup.sh',
    $maintenance_source . 'deploy.sh'
];
...
// Add maintenance scripts (location configured above)
$maintenance_files = [
    'server_setup.sh',
    'deploy.sh',
    'backup_database.sh',
...
// $maintenance_source was set above in the required files check
foreach ($maintenance_files as $file) {
    if (file_exists($maintenance_source . $file)) {
        $archive->addFile(
            'maintenance_scripts/' . $file,
            $maintenance_source . $file
        );
    }
}
...
**Verification:** Archive created and verified with all required components:
- ✅ Changed from ZIP to tar.gz format
- ✅ Includes install SQL (joinery-install.sql.gz)
- ✅ Includes config template (Globalvars_site_default.php)
- ✅ Includes all maintenance scripts (12 files)
- ✅ Total archive size: 2.04 MB with 845 files
- ✅ Both publish_upgrade.php and upgrade.php updated and working
...
- Phase 2 (Composer Dependency Management) builds on this foundation - see separate spec
