...
| S7 | Declined 2026-09-10. Moving the Refresh parse to the cron tier relocates it between two processes sharing a uid and credentials, cannot move Fortress mail at all (the level then called Fortress; now relay-sealed mail, Private + Seal at the relay), and makes Refresh wait a scheduler tick; the parse jail of separation 1 is the close — `security_inventory_closures_mail_2026_09.md` § S7 | (separation 1) | — |
...
sealed is in play** (`:174-176`). So a Private mailbox's consent
...
contacts and domain-aligned with the signature. On Private the
