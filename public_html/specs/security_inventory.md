...
| S5 | BUILT 2026-09-10, uncommitted (spec `parser_jail.md`, record `docs/document_text.md`). A setuid-root launcher (`joinery-jail`, Go, two stages) runs the extractor as its own user with no network, no forking, kernel limits and a seccomp deny-list; every outside-bytes C-parser surface (attachments, deliverability reports, received HTML, the Gmail export, the AI's fetched pages, the URL scan, the persona capture) is a `SandboxParserInterface` class run through the same subprocess, pinned by `tests/security/parser_surfaces_test.php`; the MIME parse stays in-process. Installed by a core host installer at every root moment; without it the fallback is advisory (VaultHealth + admin notice) until `JAIL_REQUIRED` flips. Fallback stays advisory (owner 2026-09-10). Open: the root install on dev, a release, the fleet upgrade | route 2, and route 3 | medium, per node |
