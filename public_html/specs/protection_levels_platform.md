...
| Fortress | stored value `fortress` means the hardened server-custody posture above | **end-to-end mail** only, per `specs/client_custody_mail.md` (ACTIVE 2026-09-24, prepared for an executor). Works with or without the relay: without it, the main server encrypts each message to the device key on arrival. No card until it exists; when it does, it states the server-side AI blackout and (relay off) the arrival exception | **build** (client_custody_mail.md) |
