...
function admin_surveys_logic(array $input): LogicResult {
	...
	$sort = LibraryFunctions::fetch_variable_local($input, 'sort', 'survey_id');
	...
	$surveys = new MultiSurvey($search_criteria, array($sort=>$sdirection), $numperpage, $offset);
	$numrecords = $surveys->count_all();
	$surveys->load();

	$page_vars = array();
	$page_vars['session'] = $session;
	$page_vars['surveys'] = $surveys;
