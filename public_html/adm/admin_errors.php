...
require_once(PathHelper::getIncludePath('adm/logic/admin_errors_logic.php'));
...
<div class="btn-toolbar mb-3" role="toolbar">
	<div class="btn-group mr-2" role="group">
		<a href="?view=recent" class="btn btn-<?= $page_vars['view'] === 'recent' ? 'primary' : 'outline-secondary' ?>">
			Recent Errors
		</a>
		<a href="?view=grouped" class="btn btn-<?= $page_vars['view'] === 'grouped' ? 'primary' : 'outline-secondary' ?>">
			Grouped View
		</a>
		<a href="?view=database" class="btn btn-<?= $page_vars['view'] === 'database' ? 'primary' : 'outline-secondary' ?>">
			Database Errors
		</a>
	</div>
</div>
...
if ($page_vars['view'] === 'database') {
	...
	foreach ($page_vars['errors'] as $error) {
		$user_name = '';
	...
	// Grouped file errors
	foreach ($page_vars['file_errors'] as $hash => $error) {
		$rowvalues = array();
	...
	foreach ($page_vars['file_errors'] as $error) {
		$rowvalues = array();
...
if (($page_vars['view'] !== 'database') && empty($page_vars['file_errors'])) {
	echo '<div class="alert alert-info">No errors found in the log file.</div>';
}

// Show parsing statistics for grouped view
if ($page_vars['view'] === 'grouped') {
	$stats = $page_vars['parsing_stats'];
