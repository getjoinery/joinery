...
// Process logic
$page_vars = process_logic(admin_subscription_tier_edit_logic(array_merge($_GET, $_POST)));
extract($page_vars);

$page = new AdminPage();

$is_edit = ($tier && $tier->key);
...
$page->begin_box(array('title' => $page_title));
...
<fieldset class="mt-4 mb-4 p-4 border border-2 rounded">
    <legend class="float-none w-auto px-3 h4">Tier Features & Limits</legend>
    <?php
...
<a href="/admin/admin_subscription_tiers" class="btn btn-secondary mt-2">Cancel</a>

<?php
$page->end_box();

if ($is_edit):
    $page->begin_box(array('title' => 'Tier Members'));
...
?>
    ...
    <?php if ($member_count > 0): ?>
        <a href="/admin/admin_subscription_tier_members?id=<?php echo $tier->key; ?>" class="btn btn-info">View Members</a>
    <?php endif; ?>
<?php
    $page->end_box();
endif;
