...
- **One plugin's installer** — `_plugin_installers_start.sh --only-plugin=<plugin>` runs one active plugin's declared `host_installer`, and nothing else, through the same checks a full run applies (path inside the plugin, root put it there, a package we built). A name that is not a plugin identifier, or `--only-plugin` beside `--machine`, `--when-changed` or `--only`, is refused with exit 2; a plugin that is not active, or declares no installer, is refused in the transcript. This is the path an agent's `run_installer plugin:NAME` takes.
- **The site's own files** — `site_housekeeping.sh`, a core installer, writes the site's `/etc/logrotate.d/joinery-{site}` and, on bare metal, its `/etc/cron.d/joinery-{site}` entry, each only when absent; `_site_init.sh` calls it at install. `host_housekeeping.sh` likewise writes `mpm_event.conf`, the journal cap and the PHP-FPM `php.ini` (rebuilt from `php.ini-production`) when absent, from `_host_files.sh`, the definition `install.sh` uses too. Absent-only, so an owner's tuning survives every converge; the agent's `reclaim_managed_file` moves one aside (keeping a dated copy) and runs its owner to put it back.
...
A publish ends by queuing a `run_plugin_installers` job on the management node's own paired agent, so the host installers a release ships (the parser jail launcher, the agent artifact, a plugin's services) converge on this machine the way an upgrade converges them on every other node; the management node's tree is the source and is never upgraded, so nothing else would run them here. A site with no agent of its own is told the one command to run instead.
...
4. **The tree fails its own tests.** The publisher runs the `deploy` tier (every deployable file compiles, the core classes load, the database answers, the manifests parse, the site answers over HTTP — seconds, all reads) — see [The deploy tier](#the-deploy-tier). The development tiers are not a publish precondition: they are the developer's working loop and pre-checkin gate ([Testing](testing.md#what-each-tier-costs)), and the publisher runs as root inside the plane's own `publish_upgrade` primitive, where they cannot run. Per-suite PASS lines are left out of the publish output; failures, skips and the summary are kept. There is no flag to skip the gate: the fix is the failing test or the code it caught.
...
7. Sync staged maintenance_scripts/ into the site root (files the release stopped shipping are removed once it passes its deploy tier)
...
a transcript sees it move in seconds. The queue is worked under the runner's
one lock (above), so two ticks inside one upgrade cannot both take it. The page that submitted the request polls
...
- **Port 443** — reads the Let's Encrypt lineage at `/etc/letsencrypt/live/${domain}/` when one exists and the **placeholder certificate** at `/etc/ssl/joinery/${domain}/` otherwise. The directory is chosen by a per-site `Define` (`JOINERY_CERT_DIR_${sitename}`, set to the placeholder and overridden inside `<IfFile>` on the Let's Encrypt path), and the block is guarded on `${JOINERY_CERT_DIR_...}/fullchain.pem`. Apache evaluates `<IfFile>` and `Define` at config-parse time, so the switch to the real certificate happens on the reload after it lands. The placeholder is a ten-year self-signed EC certificate naming the apex and `www`, minted by `_placeholder_cert.sh` (`mint_placeholder_cert`) from `write_universal_vhost` at install and from `render_vhost.sh` on every converge, only when neither file exists. It is there so the site **answers TLS from the first minute**: an edge redirects the HTTP-01 challenge to https, and a box with no `:443` listener fails that hop (525) in Full mode too, so it could never receive its first certificate. Nothing trusts the placeholder and nothing is meant to — the `:80 → https` redirect stays guarded on the Let's Encrypt path alone, so a direct visitor is never sent onto a certificate their browser will warn about, and the retry timer's `have_real_cert` refuses to count it as done. With neither file (no `openssl` at install) there is no `:443` host, no parse error, and the site serves port 80. Under the same guard the `www` name has its own `:443` host on the same certificate, answering only with a 308 to the apex, so a TLS connection whose SNI is `www.<domain>` never falls through to the first `:443` vhost on the address — on a shared host that is another site's certificate *and* another site's content.

**The origin certificate is issued for every install.** `install.sh` runs `provision_origin_cert` during install, and the rule it applies is *the name reaches this box* — which is what an HTTP-01 challenge actually requires, and which is true both when the name resolves to the box and when it resolves to an edge (Cloudflare, another CDN) that forwards to it:

1. **The name reaches this box** (`name_reaches_here`: a nonce is written where the site serves files, fetched through the name with redirects followed, and compared; it costs nothing at Let's Encrypt) → HTTP-01 through `certbot certonly --apache`, for the apex **and `www`** whenever `www` resolves and reaches here too. `--apache` answers the challenge through the running vhost; `certonly` keeps certbot from editing that vhost, which the renderer owns. A `--deploy-hook 'systemctl reload apache2'` is recorded as the lineage's `renew_hook`, so a renewal writes the files and reloads Apache and nothing else. Whether the name reached the box directly or through an edge is a word in the log, not a gate.
2. **The name does not reach this box, or HTTP-01 failed**, and a matching DNS-API credentials file exists at `/etc/letsencrypt/<provider>.ini` → LE DNS-01 with the matching certbot plugin. Plugin map:
...
3. **Neither path produces a cert** → the install proceeds on HTTP. The `:443` vhost stays dormant via the `<IfFile>` guard, and the retry timer below keeps asking.

All three DNS states at install end in the same place — a certificate for apex + www, renewing on certbot's timer, with the vhost untouched:

- **DNS points at the box:** issued during the install.
- **DNS points at Cloudflare (or another edge):** issued during the install, through the edge; the placeholder is what lets the edge's https hop complete. Cloudflare must be on **Full**, not Full (Strict), until the first certificate lands: Strict refuses the placeholder (526), so the challenge cannot get through. That is the one true limit, and the installer, the retry timer's journal line and the node's health panel all say so when they see it. The probe tells that case from the other way an https hop can fail: 525 or no answer means this box completed no handshake at all — the placeholder is missing — which is a defect on the box, named as one, with `render_vhost.sh` as the fix (`name_reaches_here` returns 2 for the first and 3 for the second).
- **DNS points nowhere yet:** the retry timer issues it once the name reaches here.
...
**Issuing or re-issuing by hand.** `sudo /var/www/html/<site>/maintenance_scripts/sysadmin_tools/setup_ssl.sh <domain>` re-enters the same decision tree. `sudo .../sysadmin_tools/issue_origin_cert.sh <domain>` is the operator's tool for a name that needs `www` added to an existing lineage, or for a container's certificate issued on its Docker host: it probes reach for both names, prints the exact certbot line, asks, passes `--expand` only when the lineage already exists, reloads Apache and verifies at the origin with SNI that each name is covered. Neither deletes anything. `sysadmin_tools/strict_readiness.sh <name>=<origin-ip> ...` is the read-only check of the same fact from outside, one line per apex and www.

**A certificate deferred at install time arrives on its own.** When no certificate landed, `arm_ssl_retry.sh` installs `joinery-ssl-retry@<domain>.timer`: every five minutes it asks `name_reaches_here`, does nothing until a request for the name lands on the box, then issues once and disables itself. A self-signed placeholder does not count as done. The reach probe is what makes an indefinite retry safe — Let's Encrypt counts failed validations, and a failed probe is one HTTP fetch that costs nothing. A restore that lands the site on a different domain re-arms the timer for the new name and disarms the old one.

**Renewal.** `install.sh server` enables `certbot.timer` (or asserts `/etc/cron.d/certbot` where there is no systemd) rather than inheriting it from the package. `host_housekeeping.sh`, on every converge, sets every lineage the machine renews through Apache to `installer = None` with a `renew_hook` that reloads Apache (`host_files_heal_renewal_confs` in `_host_files.sh`), so certbot never edits a vhost. That covers every certificate on the machine, whatever its name, and a Docker host's proxy vhosts through the host's own converger; a lineage issued by webroot or DNS is left alone, and a `renew_hook` already set is kept. In `render_vhost.sh`, the lines certbot's Apache installer inserts (an `Include` of its options file and its redirect block) are recognised as certbot's, not an operator's, so a vhost carrying them is re-rendered rather than held for a hand-apply.

### The strict-ready contract

A node guarantees this to an owner who flips Cloudflare to Full (Strict), with no operator step on the node:

- The origin presents a certificate that names the apex **and `www`** under SNI, issued over HTTP-01 through the edge, with no credential file and no operator step.
- It renews on certbot's timer, and a renewal never edits the vhost.
- When renewal stops — more than a day past the two-thirds-of-life due date with the same certificate still on disk, or the timer off, or `www` uncovered — the admin header and the health panel (`includes/CertificateNotice.php`, `VaultHealth::checkCertificates()`) say so, with the command. They read `cache/certificates.json`, which the host converger writes on every run from `/etc/letsencrypt`; a managed node's management node watches the same origin certificate over the wire (`RunNodeUptimeChecks::check_cert_expiry()`).
- It answers TLS from the first minute, with the placeholder until the real certificate lands, so an edge that redirects the challenge to https can deliver it.
- The one limit: the edge must be on Full, not Strict, until the first certificate has landed. Strict refuses the placeholder, and the HTTP-01 challenge cannot get through. The retry timer keeps asking; the health panel says to set Full until then.
- On a managed node the management node also says when the origin answers with a certificate for other names — the placeholder, or a shared fallback vhost — because that is exactly what Strict would reject.
- The edge must let the challenge through. Cloudflare's Under Attack mode, or a firewall rule that challenges every request, stops Let's Encrypt's validator at the edge; renewal fails while that is on, the alerts say so, and nothing on the node can work around it. Outside that, the encryption mode does not matter once a certificate exists: Flexible, Full and Full (Strict) all pass the challenge.
...
3. Publish Release on This Node, on the same tab. getjoinery's own agent builds the archives from the release it now runs (`specs/publish_as_node_action.md`).
...
**A site that republishes serves the release it received, file for file.** Every archive — the core, each plugin and each theme — is staged from exactly the files its received, signed `RELEASE_MANIFEST` lists, each copy checked against the listed hash (`TreeManifestPublisher::republish_artifact()`), and the manifest ships alongside unchanged. A file in the tree that no manifest lists does not ship. A listed file that is missing, or whose bytes differ, refuses the publish and names the file; upgrading the site again restores it. Nothing is regenerated or rebuilt first: the install SQL is the received one (never dumped from the republishing site's own database), `VERSION` already reads the running version, component versions are not auto-bumped, and the agent, relay-sealer, parser-jail and support-bundle builds do not run. The archive's `config/default_Globalvars_site.php` is copied from the manifest-checked `maintenance_scripts/install_tools/` template, as the authoring site's is.
