...
4. **The tree fails its own tests.** The publisher runs the `deploy` tier (every deployable file compiles, the core classes load, the database answers, the manifests parse, the site answers over HTTP — seconds, all reads) — see [The deploy tier](#the-deploy-tier). The development tiers are not a publish precondition: they are the developer's working loop and pre-checkin gate ([Testing](testing.md#what-each-tier-costs)), and the publisher is root on the local job queue, where they cannot run. Per-suite PASS lines are left out of the publish output; failures, skips and the summary are kept. There is no flag to skip the gate: the fix is the failing test or the code it caught.
