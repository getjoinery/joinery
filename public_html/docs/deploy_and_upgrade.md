...
**Renewal.** `install.sh server` enables `certbot.timer` (or asserts `/etc/cron.d/certbot` where there is no systemd) rather than inheriting it from the package. `render_vhost.sh`, on every converge, sets the site's lineage to `installer = None` with a `renew_hook` that reloads Apache, so certbot never edits the vhost; the lines certbot's Apache installer inserts (an `Include` of its options file and its redirect block) are recognised as certbot's, not an operator's, so a vhost carrying them is re-rendered rather than held for a hand-apply.
