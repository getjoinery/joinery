...
An installer that holds a sending key does the task itself. `utils/install_mail_provider.php` (CLI-only; inputs in the environment: `JOINERY_MAIL_API_KEY`, optional `JOINERY_MAIL_PROVIDER` — otherwise detected from the key through `EmailSender::providersForApiKey()`, each candidate tried live in turn — `JOINERY_MAIL_FROM`, `JOINERY_ADMIN_EMAIL`) runs the setup wizard's own Email ceremony through the same functions the wizard calls: it writes the provider and From address, validates the key against the provider, provisions the owner's mailbox for the address, registers the From domain at the provider, publishes the domain's mail records through the DNS credential the installer kept (`DnsInstallCredential`, consumed on use), and asks the provider to verify. Only providers a single key configures qualify — those implementing `SingleKeyProvider`, which declares the shape of their keys; SMTP is refused. Registration at the provider happens where it is also a `SendingDomainRegistrar`. A key the provider rejects puts every setting back to its previous value. The wizard then opens on the delivery proof, which stays a human step. The Linode StackScript runs it when the deploy form's SMTP2GO key field is filled in.
