...
Two channels are not keys at all and do not appear in that table. The **relay birth channel**, `/api/v1/relay/*`, is how a relay born from first-boot user-data fetches its bundle (`GET relay/bundle?run=&sha256=`) and posts its signed birth report (`POST relay/born`); its credential is a one-time run token in `X-Joinery-Relay-Run-Token` plus, for the report, an Ed25519 signature by the relay's own identity key, and the plane believes the report only after the provider's address answers a pinned ping (see [Mailbox § Relay-fronted receiving](../plugins/mailbox/docs/overview.md)). It is dispatched before key authentication, metered in its own bucket, and exists only where the `mailbox` plugin is active. The other is the **agent channel**. `/api/v1/agent/*` is how a managed node's Go agent takes work from its management node, and it authenticates with an Ed25519 signature over a canonical message rather than a shared secret — the plane stores only the node's public key, so it holds nothing that could act as the node. It is dispatched before key authentication, has its own rate-limit bucket (`api_agent_rate_limit_requests`), and exists only where the `server_manager` plugin is active. See [Server Manager § The agent channel](../plugins/server_manager/docs/overview.md#the-agent-channel).
