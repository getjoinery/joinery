...
class RouteHelper {
     ...
     * (/profile, /drive, /notifications, and everything beneath them,
     * including plugin member namespaces like /profile/mailbox). The single
     * source for this boundary: the member nav
     * (PublicPageBase::in_member_area()) and the member-area app chrome pin
     * (PathHelper::getActiveThemeDirectory(), specs/member_area_app_chrome.md)
     * both consult it.
     */
    public static function isMemberAreaPath($path = null) {
        ...
        return (bool)preg_match('#^/(profile|drive|notifications)(/|$)#', $path);
    }
