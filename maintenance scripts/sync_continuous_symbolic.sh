...
# Fixed version of watch_with_inotify function
watch_with_inotify() {
    ...
    if [ -L "$LOCAL_DIR" ]; then
        ...
        print_status "Resolved symlink: $LOCAL_DIR -> $resolved_dir"
    else
...
# Performance-optimized version of watch_with_polling function
watch_with_polling() {
    ...
    # Find all symlinks and their targets once
    ...
    # Build single find command for better performance
    local find_paths=()
    
    # Add main directory with exclusions
    find_paths+=("$monitor_dir")
    ...
    for path in "${find_paths[@]}"; do
        if [ "$path" = "$monitor_dir" ]; then
            # Exclude symlinked subdirs from main directory count
            total_files=$((total_files + $(find "$path" -maxdepth 20 -type f -not -path "*/.*" 2>/dev/null | grep -v "$monitor_dir/plugins" | grep -v "$monitor_dir/theme" | wc -l)))
        else
            total_files=$((total_files + $(find "$path" -type f 2>/dev/null | wc -l)))
        fi
    done
    
    print_status "Monitoring $total_files files across ${#find_paths[@]} directories"
    ...
    # Store initial state using optimized method
    local last_state_file="/tmp/.sync_state_$$"
    
    # Single combined find operation for better performance
    {
        # Main directory (excluding symlinked subdirs)
        find "$monitor_dir" -maxdepth 20 -type f -not -path "*/.*" 2>/dev/null | \
            grep -v "$monitor_dir/plugins" | grep -v "$monitor_dir/theme"
        ...
        if [ -f "$file" ]; then
            # Use lighter stat format - just modification time and size
            stat -c '%n %Y %s' "$file" 2>/dev/null
        fi
    done | sort > "$last_state_file"
    
    local scan_count=0
    local last_scan_time=$(date +%s)
    ...
    while [ "$SHOULD_EXIT" = false ]; do
        ...
        # Get current state using the same optimized method
        ...
        # Quick file count comparison first (faster than full diff)
        ...
        elif ! cmp -s "$last_state_file" "$current_state_file"; then
            # Files modified but count same - use faster cmp instead of diff
            changes_detected=true
        fi
        ...
        if [ "$changes_detected" = true ]; then
            if [ "$VERBOSE" = true ]; then
                ...
                comm -23 "$last_state_file" "$current_state_file" | head -3 | while read line; do
                    echo "    - $line"
                done
            fi
            ...
            # Show scan performance occasionally
            if [ "$VERBOSE" = true ] || [ $((scan_count % 10)) -eq 0 ]; then
