 ...
 * server-driven account forms, and sign out.
 *
 * Phase 1 has no webview, so the App Sessions link (a webview destination) is
 * added in Phase 2 alongside the navigation shell.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SettingsScreen(session: SessionController, user: UserSummary) {
    ...
    Scaffold(topBar = {
        ...
        when (route) {
            SettingsRoute.Root -> SettingsRoot(session, user, content) { route = it }
...
private enum class SettingsRoute { Root, AccountEdit, ContactPreferences, PasswordEdit }
