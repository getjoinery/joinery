<?php
	...
	echo PublicPageSassa::BeginPage('Change Subscription', $hoptions);
...
*/
	echo PublicPageSassa::tab_menu($tab_menus, 'Change Subscription');
...
if($_GET['new_plan']){
	...
	echo '<p>You are about to change your subscription to the '.$page_vars['product']->get('pro_name').' </p>';
	...
	==============================-->
						   ...
						   <script>
							...
							toggle.addEventListener('change', function () {
							  ...
							  if (this.checked) {
								...
								window.location.href = '/subscription_edit?page=year';
							  } else {
					...
					<div class="row justify-content-center">
						...
						<div class="col-xl-4 col-md-6">
							<div class="price-box th-ani ">
								...
								echo $product->get('pro_recurring'); 
								?></span></h4>
								...
								<div class="box-content">
									...
									<a href="/profile/subscription_edit?new_plan=<?php echo $product->key; ?>" class="th-btn btn-fw style-radius">Choose Plan</a>
								</div>
							</div>
						</div>
						<?php } ?>
		...
		<?php 
		/*
		echo '<h2 class="sec-title">New Plan</h2>';
			
		
		
		echo $formwriter->begin_form("", "post", "/profile/subscription_edit");
		

		foreach ($page_vars['products'] as $product){
			$name = $product->get('pro_name').$product->get_readable_price();
			$optionvals[$name] = $product->key;
		}

		
		echo $formwriter->dropinput("New Plan", "new_plan", NULL, $optionvals, NULL, '', FALSE);
		//echo $formwriter->hiddeninput('order_item_id', $_GET['order_item_id']);
